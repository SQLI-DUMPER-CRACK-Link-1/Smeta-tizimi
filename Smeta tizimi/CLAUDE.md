...
| # | Muammo | Tuzatish |
|---|--------|---------|
| 69 | **⚡⚡⚡ F2 Импорт: HECH BIR razdel to'g'ri ochilmasdi (tasdiqlangan, real brauzerda reproduksiya qilindi)** | Avvalgi #68 tuzatish YETARLI EMAS edi — asosiy sabab boshqa joyda ekan. `apiF2FaylOqi` (30_Panel.js, F2 fayl import qilib daraxt qurish) — `bl`/`rs`/`mat`/`ob` tugunlarga `uid:'f2_'+i` berilardi, lekin **`rz` (razdel) tugunlarga UMUMAN uid berilmagan edi**. Panel.html `drawF2Node`: `cId='f2cc_'+node.uid` — rz uchun `node.uid===undefined` bo'lgani sabab **BARCHA razdellar bir xil `id="f2cc_undefined"` olgan edi** (izolyatsiyalangan test-sahifada aniq reproduksiya qilindi: `document.querySelectorAll('[id="f2cc_undefined"]').length` → 3). Natijada `document.getElementById()` DOIM birinchi (DOM'dagi eng yuqori, odatda "Асосий бўлим" default konteyner) elementni qaytargani uchun — istalgan razdel bosilganda ALBATTA birinchisi ochilib/yopilib qolardi, qolganlari umuman ishlamasdi. Tuzatish: har bir rz yaratilganda ham `uid:'f2rz_'+(_rzSeq++)` beriladi (bl/rs bilan bir xil pattern). Test-sahifada FIX dan keyin har bir razdel mustaqil, to'g'ri ochilishi tasdiqlandi (`querySelectorAll` → 1 ta mos element) |
| 68 | **Ф2/Ҳолат дарахтида баъзи "razdel" бошилса ичи очилмасди** (GAME CLUB) | `apiHolatOl` (30_Panel.js) — LRV da ketma-ket bir nechta `rz` markerli qator bo'lsa (masalan "GAME CLUB" → "АР И КЖ" → "ЗЕМЛЯНЫЕ РАБОТЫ" — dekorativ sarlavhalar, haqiqiy ish faqat oxirgisida boshlanadi), har biri ALOHIDA, bola ololmaydigan BO'SH razdel sifatida daraxtga qo'shilardi (chunki `curRz` keyingi rz kelganda darhol almashtirilib, hech qachon child olmasdi) — bosilganda haqiqatan ham bo'sh bo'lgani uchun "ichi ochilmaydi" bo'lib ko'rinardi. Endi: (a) ketma-ket, hali bola olmagan rz avtomatik almashtiriladi (oldingi bo'sh rz o'chiriladi, faqat oxirgi/haqiqiy razdel qoladi); (b) varaq oxirida ham bo'sh qolib ketgan rz'lar yakuniy filtr bilan tozalanadi |
| 67 | **F2 Импортда номзодларни қидириш қийин эди** | Panel.html — F2 faylidan ish turi ketma-ket kiritilayotganda, aynan shu ish/resurs smetaning QAYERLARIDA ishlatilganini topish uchun butun daraxtni qo'lda titkilash kerak edi. Endi har bir mos kelmagan (yoki bir nechta nomzodli — noaniq) F2 qatorida **"🔍 N та жой"** tugmasi chiqadi (`_f2Candidates` — xuddi shu KOD yoki normallashtirilgan НОМ+БИРЛИК bo'yicha, band qilinmagan smeta qatorlari) — bosilsa ro'yxat ochiladi, birini tanlasa (`f2TanlaNomzod`) drag-drop bilan bir xil natijada bog'lanadi. `f2AvtoMoslash`dagi indeks (`_f2ByKod`/`_f2ByNomBir`) qayta ishlatiladi — ikkalasi bir xil CONSTANTA (aniq moslik, fuzzy yo'q) qoidasidan foydalanadi |
| 66 | **Ф2 Импорт — қўлда drag-drop кўп вақт олади** | Panel.html: yangi `f2AvtoMoslash()` — F2 import daraxti qurilgach avtomatik (va tugma orqali qo'lda ham) ishga tushadi. Smeta daraxtini КОД va normallashtirilgan НОМ+БИРЛИК (CONSTANTA — narxlashdagi `_normNomKey`/`_normBirlik` bilan bir xil mantiq, JS'da qayta yozilgan) bo'yicha indekslaydi, F2 daraxtidagi har bir bl/rs/mat/ob uchun **FAQAT BITTA aniq nomzod topilsa** (kod yoki nom+birlik bo'yicha, boshqa hech qanday nomzod bilan raqobatlashmasa) — avtomatik bog'laydi, narxni ham (agar F2 faylda bo'lsa) o'tkazadi. Noaniq (bir nechta nomzod yoki hech qaysi mos kelmasa) — TEGILMAYDI, qo'lda (drag-drop yoki +Доп) qoldiriladi — fuzzy/taxminiy moslik YO'Q (narxlash CONSTANTA qoidasiga mos). Natijada foydalanuvchi faqat murakkab/noaniq qatorlarni qo'lda hal qiladi, aksariyati avtomatik. Chap/o'ng panelda НАРХ+СУММА ham ko'rsatiladi (tasdiqlash uchun) |
| 65 | **Ф2 Импорт (икки ойнали тизим) — мослаштирилган (mapped) қаторлар учун НАРХ йўқолиб кетарди** | Bu — allaqachon mavjud, kuchli F2 import vositasi: davlat aktini (Excel/Sheets) yuklab, avtomatik daraxt qurib, drag-drop bilan joriy smetaga moslashtiradi. Tekshirilganda: FAQAT yangi "+ Доп" ishlar uchun НАРХ to'g'ri o'tar edi (`_f2Dopps`); lekin mavjud smeta qatoriga MOSLASHTIRILGAN (asosiy, eng ko'p ishlatiladigan) holatda `_f2Mappings` obyektida НАРХ umuman yo'q edi — faqat `hajm` saqlanardi. Natijada `apiF2Qolla` doim `oyl[oyNom]=e.hajm` (oddiy son) yuborardi, `apiHolatSaqla` esa НАРХ ustuniga tegmasdi — davlat aktidagi haqiqiy narx yo'qolib, har doim smeta narxi (=G) qolib ketardi. Tuzatildi: (a) `_f2Mappings.push(...)` uchtala joyda ham `narx:` maydoni qo'shildi (Panel.html); (b) `apiF2Qolla` endi `{obyom:e.hajm, narx:e.narx}` shaklida yuboradi (narx>0 bo'lsagina — fix #64 dagi {obyom,narx} tizimidan foydalanadi); (c) chap panel (import qilingan F2 hujjati) va o'ng panel (moslashtirilgan qatorlar) endi НАРХ+СУММАni ham ko'rsatadi — foydalanuvchi saqlashdan oldin aktdagi raqam bilan solishtirib ko'ra oladi |
| 64 | **F2 fakticheskiy НАРХ Panel'da tahrirlanmasdi** (faqat ҲАЖМ kiritilardi) | Fix #47 da har F2 oyi uchun 3-ustunli tizim (ОБЪЁМ\|НАРХ\|СУММА) LRV'da qo'shilgan edi, lekin Panel (`apiHolatOl`/`apiHolatSaqla`) faqat ОБЪЁМ ustunini o'qirdi/yozardi — НАРХ har doim formula bo'yicha (=G, smeta narxi) qolib, foydalanuvchi davlat aktidagi ANIQ (fakticheskiy, so'mning tiyinigacha) narxni kirita olmasdi. Endi: `apiHolatOl` НАРХ ustunini ham o'qiydi + formula/qiymat farqini aniqlaydi (`narxIsFormula`); Panel Ҳолат tabida har oy qatoriga 2-input qo'shildi — ҲАЖМ (объём) va НАРХ (bo'sh=смета нархи, foydalanuvchi kiritsa=fakticheskiy), yoniga жонли СУММА (=ҳажм×нарх) preview. `apiHolatSaqla` НАРХ ustuniga FAQAT foydalanuvchi aniq kiritganda yozadi (aks holda formula/smeta-narxga bog'liqligi buzilmaydi). Natija: F2 aktdagi har bir raqamni (ҳажм ҳам, нарх ҳам) so'mning tiyinigacha aniq kiritish mumkin; FAKT (umumiy bajarilgan ҳажм) va oylik F2 (даврий даъво) bir xil панелда, bir joyda boshqariladi |
| 63 | **Қўшимча ишлар (rs+/mat+/ob+) асл сметага аралашиб кетарди (Ташхис)** | `apiTashxis` (30_Panel.js) — avval `+` belgisi darhol tashlanib, qo'shimcha ishlar summasi `jamiSmeta`ga qo'shilib ketardi (masalan 59mlrd o'rniga 64mlrd ko'rinardi). Endi `isQosh` alohida kuzatiladi: `jamiSmeta`=FAQAT asl smeta, yangi `qoshSmeta`/`qoshKatSum`=FAQAT qo'shimcha (kategoriya bo'yicha ham). Panel Ташхис oynasida ikkalasi ALOHIDA qatorda ko'rsatiladi ("ЖАМИ СМЕТА (асл)" va "➕ ҚЎШИМЧА ИШЛАР"). ⚠️ Chuqurroq qism (LRV ichidagi BL SMETA rollup formulasi va DASHBOARD/накрутка manbai) hali ham ikkalasini birlashtirib hisoblaydi (`_qoshQayta` BL SMETA=SUM barcha bola, qo'shimcha ham kiradi) — bu keyingi bosqichda alohida ustunga chiqarilishi kerak, hozircha faqat Ташхис darajasida ajratildi |
| 62 | **Har obyekt uchun НАКРУТКА (ПРЯМЫЕ→ВСЕГО) hisob-kitobi** | GAME CLUB'ning F2 akt fayli ichidagi LRV varag'i podvalidan (qator 504-523) накрутка zanjirining aniq foizlari va formulalari o'qib olindi — 80_Shartnoma.js dagi mavjud `nakrutkaHisob()` bilan 100% mos keldi (ТРАНСПОРТ_МАТЕРИАЛ 5%, СКЛАДСКИЕ_МАТЕРИАЛ 2%+М/К 0.75%, ТРАНСПОРТ_КАБЕЛЬ 1.5%, ПРОЧИЕ_ПОДРЯДЧИК 18%, ТРАНСПОРТ_ОБОРУД 2%, ЗАГОТ_СКЛАД_ОБОРУД 1.2%, СТРАХОВАНИЕ 0.32%, НДС 12%). Avval bu faqat ШАРТНОМА (dogovor) darajasida hisoblanardi (`apiShartnomaDashboard`). Endi **har OBYEKT uchun alohida**: yangi `apiObyektNakrutka(obyekt)` (80_Shartnoma.js) — DASHBOARD dan kategoriya jamilarini (ЧЕЛ/МАШ/МАТ/ОБ/М-К/КАБ) olib, obyekt biror shartnomaga biriktirilgan bo'lsa o'sha накрутка override bilan, aks holda default bilan hisoblaydi. Panel: Ҳолат tab → **[💰 Наркутка]** tugmasi → to'liq 20-qatorli zanjir (ИТОГО ПРЯМЫЕ ЗАТРАТЫ → ... → ВСЕГО), pastida ЖАМИ ХАРАЖАТ/МИЖОЗГА НАРХИ/НАКРУТКА СУММАСИ (фойда) va har kategoriya uchun "накруткасиз/накрутка билан" taqribiy narx |
| 61 | **Duplikat navbat → "A sheet with ID ... does not exist"** (Monitoringda tushunarsiz xato) | `navbatBoshla` (50_Navbat.js): navbat ishlab turganda bitta obyekt qo'lda qo'shilsa (masalan "⚡" tugmasi), faqat HALI KUTAYOTGAN ro'yxatdan (`_NB_KEY`) tekshirilardi — obyekt ALLAQACHON shu partiyada muvaffaqiyatli ishlangan (lekin navbatdan chiqarilgan) bo'lsa ham qayta qo'shilardi → ikki marta ishlanib, 2-marta GAS ichki sheet-keshi eskirgani uchun shu xato chiqardi (ma'lumotga zarar yo'q edi). Endi `_NB_LOG` dan "shu partiyada allaqachon ok:true bilan tugagan" obyektlar chiqarib tashlanadi — qayta qo'shilmaydi. Qolganicha (masalan eski partiyalarda) shu xato chiqsa — endi `_navbatQadam` catch blokida tushunarli o'zbekcha izoh bilan ko'rsatiladi ("объект бошқа жараёнда ишланган, зарар йўқ, хавфсиз"). Monitoring logiga har obyekt tugaganda MISS soni ham qo'shildi (`⚠ N та нарх топилмади`) |
| 60 | **`rs` turdagi resurs MISS umuman logланмасди** (barcha obyektlarda, tizim darajasida) | `_ishlaVaraq` (10_Engine.js) da `_NARX_LOG` ga yozish faqat `mat`/`ob` branchida bor edi (qattiq kodlangan `'mat'` literal bilan — `ob` qatorlar ham noto'g'ri 'mat' deb yozilardi), `rs` branch (BL ostidagi ЧЕЛ/МАШ/material resurslar) esa MISS bo'lsa ham logga HECH NARSA yozmasdi — jimgina 0 bo'lib qolardi, izsiz. Endi: (a) `rs` branchga ham xuddi shunday MISS/⚠ log qo'shildi; (b) ТУР ustuni haqiqiy `tur` qiymati bilan yoziladi (mat/ob/rs farqlanadi); (c) izoh "Свод топилмади: <калит>" ko'rinishida aniqroq; (d) `_tezkorObyekt`/`_narxYangilaVaraq` ("⚡ Тезкор нарх" tugmasi) ham endi to'liq Ишла kabi `_NARX_LOG` ga yozadi — avval faqat sonini (`topilmadi`) qaytarib, sahifada iz qoldirmasdi |
| 59 | **Oraliq tashqarisidagi ЧЕЛ-Ч/МАШ-Ч butunlay tashlanardi** (Sun'iy ko'l — skvajina bo'limi 0-narx) | `_priceDB` (10_Engine.js) — `hasRanges` bo'lganda avval `_oraliqKat` tekshirilib, oraliq tashqarisidagi HAR QANDAY qator (ЧЕЛ/МАШ ham) `continue` bilan narx bazasidan tashlanardi. Agar bo'lim (masalan skvajina) hali SOZLAMALAR_ОРАЛИҚ da belgilanmagan bo'lsa — o'sha bo'limning ЧЕЛ-Ч/МАШ-Ч resurslari HAM narx bazasiga kirmay, keyin `_findPrice` MISS→0 qo'yardi. Endi tartib teskari: avval `_catBirlik` orqali ЧЕЛ/МАШ aniqlansa — oraliqqa QARAMASDAN shu kategoriyaga tushadi; oraliq chegarasi FAQAT ЧЕЛ/МАШ bo'lmagan (mat/ob/boshqa rs) qatorlarga qo'llanadi |
