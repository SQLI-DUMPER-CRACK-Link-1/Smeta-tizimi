...
import { Briefcase, Plus, FileSignature, AlertCircle, Search, FileText, CheckCircle2, ChevronRight, Calculator, PieChart } from 'lucide-react';
...
export default function TestShartnoma() {
  ...
  // MOCK DATA for B2B Contract Management (Phase 3)
  const shartnomaList = [
    { id: 1, raqam: '№-24/01', nomi: 'Bosh Pudrat (Yangi Obyekt)', taraf: 'Davlat Buyurtmachisi', summa: 12500000000, nds: true, avans: 30, retention: 5, holat: 'imzolangan', sana: '2026-08-01' },
    { id: 2, raqam: '№-SUB-12', nomi: 'Monolit ishlari', taraf: 'Stroy-Master MCHJ', summa: 2400000000, nds: false, avans: 15, retention: 5, holat: 'kutilmoqda', sana: '2026-08-15' },
    { id: 3, raqam: '№-XAR-05', nomi: 'Sement va Armatura yetkazish', taraf: 'Toshkent Sement', summa: 850000000, nds: true, avans: 100, retention: 0, holat: 'imzolangan', sana: '2026-08-20' },
  ];
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-text-dim text-sm max-w-2xl">
            Bosh pudrat, sub-pudrat, va xarid shartnomalarining markazlashgan reestri. Qo'shimcha kelishuvlar (Dop. Soglasheniye), ustamalar va bo'nak (avans) nazorati. (Faza-3)
          </p>
        </div>
        <button 
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white focus:border-purple-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-dim mb-1">Sana</label>
                  <input type="date" className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white focus:border-purple-500 outline-none" />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="text" placeholder="INN orqali qidiring..." className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white focus:border-purple-500 outline-none" />
              </div>

              <div className="grid grid-cols-2 gap-4 border-t border-border pt-4">
                <div>
                  <label className="block text-sm font-medium text-text-dim mb-1">Summa (UZS)</label>
                  <input type="text" placeholder="0.00" className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white font-mono focus:border-purple-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-dim mb-1">Soliq Toifasi</label>
                  <select className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white focus:border-purple-500 outline-none">
                    <option>QQS (12%) bilan</option>
                    <option>QQS siz (0%)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-text-dim mb-1">Bo'nak (Avans) %</label>
                  <input type="number" defaultValue="30" className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white font-mono focus:border-purple-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-text-dim mb-1">Kafolat Depoziti (Retention) %</label>
                  <input type="number" defaultValue="5" className="w-full bg-bg border border-border rounded-lg px-3 py-2 text-white font-mono focus:border-purple-500 outline-none" />
                </div>
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-border mt-4">
                ...
                <button onClick={() => {toast("Shartnoma saqlandi", "ok"); setIsFormOpen(false)}} className="bg-purple-600 hover:bg-purple-500 text-white px-5 py-2 rounded-lg font-medium transition-colors shadow-lg shadow-purple-900/20">Saqlash</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="bg-surface-2 border-b border-border text-text-dim font-medium">
                ...
                <th className="px-6 py-4 text-center">Avans/Ret</th>
            ...
            <tbody className="divide-y divide-border">
              {shartnomaList.map(s => (
                <tr key={s.id} className="hover:bg-bg/50 transition-colors">
                  <td className="px-6 py-4 font-mono font-bold text-purple-400">{s.raqam} <br/><span className="text-[10px] text-text-dim font-sans">{s.sana}</span></td>
                  ...
                  <td className="px-6 py-4 text-center font-mono text-xs">
                    <div className="text-sky-400">{s.avans}% Avans</div>
                    <div className="text-rose-400">{s.retention}% Ret</div>
                  </td>
                  <td className="px-6 py-4 text-center">
                    {s.holat === 'imzolangan' ? (
                      <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider flex items-center justify-center gap-1"><CheckCircle2 size={12}/> E-Imzo</span>
                    ) : (
                      <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider flex items-center justify-center gap-1"><AlertCircle size={12}/> Kutilmoqda</span>
                    )}
                  </td>
                </tr>
              ))}
