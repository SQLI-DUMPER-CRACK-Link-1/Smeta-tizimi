...
export function F2TayyorlashNative() {
  ...
  return <Sahifa sarlavha="F2 tayyorlash" tavsif="Fakt qoldig‘idan qoralama; narx va summa faqat F2 manbasidan">
    <div className="flex h-full min-h-0 flex-col gap-3">
      <section className="lux-karta flex flex-wrap items-end gap-3 p-3">
      ...
      {!validId && <section className="lux-karta p-4 text-text-dim">Avval obyektni kanonik ro‘yxatdan tanlang.</section>}
      {error && validId && <section role="alert" className="lux-karta flex flex-wrap items-center gap-3 border-danger/40 bg-danger/5 p-4 text-[13px] text-danger"><AlertTriangle size={16} /><span className="flex-1">{error}</span><button type="button" onClick={() => void yuklash()} className="lux-btn lux-btn-ghost !text-red-400 !border-red-500/50">Qayta urinib ko‘rish</button></section>}
      {loading && <div className="lux-karta animate-pulse min-h-[260px] flex-1 rounded-xl" />}
      {validId && !loading && !error && <>
        <section className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6" aria-label="F2 tayyorlash xulosasi">
          ...
          ].map(([label, value, tone]) => <div key={String(label)} className="lux-karta px-3 py-2"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-text-mute">{label}</p><p className={`mt-1 text-lg font-semibold tabular-nums ${tone}`}>{value}</p></div>)}
        </section>
        ...
        <section className="lux-karta min-h-0 flex-1 overflow-auto">
