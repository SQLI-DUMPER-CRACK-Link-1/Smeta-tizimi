...
function Sessiya() {
  ...
  async function eksportQilish() {
    ...
    if (!hisob.valid) { setError(`Eksport to‘xtatildi: ${hisob.muammolarSoni} ta qator tekshiruvdan o‘tmadi. Qizil qatorlarni to‘ldiring.`); return; }
    setBusy(true);
    try {
      ...
      setMessage('Asl RES faylining barcha varaqlari saqlandi. Faqat tanlangan RES/transport varaqlariga pudratchi narxi va taklif summasi ustunlari qo‘shildi.');
    ...
    {qatorlar.length > 0 && <>
      <section className="karta space-y-3 p-3">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button type="button" className="tugma tugma-asosiy h-9" onClick={() => void eksportQilish()} disabled={busy || !hisob.valid}><Download size={14} /> Asl RES faylini oferta qilish</button>
        </div>
        ...
        <div className={`rounded-lg border p-2 ${hisob.valid ? 'border-ok/40 bg-ok/5' : 'border-warn/40 bg-warn/5'}`}><span className="text-[10px] text-text-mute">Tekshiruv</span><strong className="mt-1 block text-sm text-text">{hisob.valid ? 'TAYYOR' : `${hisob.muammolarSoni} ta muammo`}</strong></div>
      </section>
        ...
        <div className="max-h-[58vh] overflow-auto rounded border border-border/60">
          <table className="w-full min-w-[1240px] text-[11px]">
            ...
            <tbody>{korinadigan.map((qator) => <tr key={qator.sourceId} className="border-t border-border/50 align-top">
              ...
              <td className="px-2 py-1.5 text-right tabular-nums text-text">{qator.turi === 'bolim' || qator.turi === 'jami' ? '—' : qator.hajm == null
              ...
              <td className="bg-yellow-50 px-2 py-1">{qator.turi === 'jami' || qator.turi === 'bolim' ? '—' : <input aria-label={`${qator.nom} pudratchi ${qator.hisobTuri === 'manba_jami' ? 'summasi' : 'narxi'}`} inputMode="decimal" value={manualNarxlar[qator.sourceId] ?? ''} onChange={(e) => narxniOzgartir(qator.sourceId, e.target.value)} placeholder={qator.hisobTuri === 'manba_jami' ? 'Taklif summasi' : fmt(qator.pudratchiBirlikNarx)} className="w-32 rounded border border-border bg-white px-1.5 py-1 text-right tabular-nums" />}</td>
