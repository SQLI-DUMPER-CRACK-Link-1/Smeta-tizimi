...
const RES_BOLIM_NAQSH: ReadonlyArray<readonly [RegExp, T2ResursKategoriya]> = [
  ...
  [/^(СТРОИТЕЛЬНЫЕ\s+)?МАТЕРИАЛЫ(,?\s+ИЗДЕЛИЯ)?(\s+И\s+(КОНСТРУКЦИИ|ИЗДЕЛИЯ))?$/, 'МАТ'],
  /* Pomoshnik PTO (VBA, 2026-09-25 tahlili) lug'atidan: ABC/TN eksportlarida
     uchraydigan sarlavha variantlari — faqat TO'LIQ moslik. */
  [/^МЕСТНЫЕ\s+МАТЕРИАЛЫ(\s+И\s+КОНСТРУКЦИИ)?$/, 'МАТ'],
  [/^КАБЕЛЬ(НАЯ\s+ПРОДУКЦИЯ|НЫЕ\s+ИЗДЕЛИЯ|НЫЕ\s+РЕСУРСЫ)?$/, 'КАБ'],
  [/^ПРОВОДА?\s+И\s+КАБЕЛИ$/, 'КАБ'],
  [/^КАБЕЛИ\s+И\s+ПРОВОДА$/, 'КАБ'],
  [/^КОНСТРУКЦИИ\s+ЗАВОДСКОГО\s+ИЗГОТОВЛЕНИЯ$/, 'М/К'],
  [/^(МЕТАЛЛО)?КОНСТРУКЦИИ$/, 'М/К'],
  [/^МЕТАЛЛИЧЕСКИЕ\s+КОНСТРУКЦИИ$/, 'М/К'],
