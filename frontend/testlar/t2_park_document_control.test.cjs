...
const legacy = read('src/lib/park-document-control/legacy-compat/normalizer.ts');
const report = read('src/lib/park-document-control/legacy-compat/report-generator.ts');
...
assert.match(legacy, /legacy-migration:/);
assert.match(legacy, /row\.marker === 'rs'/);
assert.match(report, /JSON\.stringify/);
console.log('PARK document-control guards: 11 checks passed');
