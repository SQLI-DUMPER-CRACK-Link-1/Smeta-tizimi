...
2. **50k-row resumable job model** (owner requirement §5) — **DRAFTED, UNAPPLIED,
   ...
   `supabase/migrations/20260922120000_t2_f2_import_job_v1.{sql,rollback.sql,acceptance.sql}`
   (renamed from 20260914120000 during integration review -- that timestamp
   was already used by an unrelated migration on another task's branch).
