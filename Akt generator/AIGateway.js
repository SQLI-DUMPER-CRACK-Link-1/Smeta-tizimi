...
function aiFetchRaw(model, payload, opts){
  ...
  // Limit to 2 retries to avoid Apps Script 6-minute maximum execution time limit
  var maxRetry=_aiCfg('AI_MAX_RETRY', 2);
  ...
  for(var mi=0; mi<models.length; mi++){
    ...
    for(var i=0;i<maxRetry;i++){
      ...
      if(qaytarsa){
        if(i<maxRetry-1){ 
          var delay = _aiBackoff(json,i);
          var reqId = PropertiesService.getScriptProperties().getProperty('CURRENT_REQ_ID');
          if(reqId && typeof aktLogStep === 'function') {
             aktLogStep(reqId, "Google AI band. Yana " + Math.round(delay/1000) + " soniya kutilmoqda (" + (i+1) + "/" + maxRetry + ")...");
          }
...
/** Smeta Panel dan saqlangan kalitni SOZLAMALAR varag'idan o'qib sinxronlash */
var _SMETA_SS_AI = '18mixKyl59e7spYtTZEBdseRIeH5JnjWdHehoCIvJqos';
function aktAiKalitSmetaSync(){
  try{
    ...
    if(key.length<15) return 'Smeta da kalit yo\'q — avval Panel → Созлама dan kiriting';
