-- PRE-USE ONLY rollback.  Never run after real contract scope decisions exist.
-- The two existing nakrutka functions were replaced by this migration, so
-- rollback restores their preceding definitions before removing the new
-- scope layer.  It never drops an existing calculation function blindly.
begin;

do $$
begin
  if exists (select 1 from public.t2_shartnoma_qator_qamrov limit 1) then
    raise exception 'ROLLBACK_REFUSED: contract scope rows already exist';
  end if;
end $$;
...
begin
  ...
  if v_shartnoma_id is null then
    select b.shartnoma_id into v_shartnoma_id from public.t2_shartnoma_bog b
  ...
  select jsonb_build_object(
    ...
    'kab',coalesce(sum(summa) filter(where kat='КАБ'),0),'bez',0)
...
drop function if exists public.t2_shartnoma_qamrov_saqla_v1(bigint,bigint,bigint,bigint,bigint,text,numeric,text,bigint,uuid,integer);
drop function if exists public.t2_shartnoma_qamrov_ol_v1(bigint,bigint);
drop table if exists public.t2_shartnoma_qator_qamrov;
