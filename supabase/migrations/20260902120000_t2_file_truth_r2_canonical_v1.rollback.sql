...
drop function if exists public.t2_document_canonical_upsert_v1(bigint,bigint,bigint,bigint,text,text,text,bigint,text,text,text,uuid,text);
...
alter table public.t2_document_registry
  ...
  drop column if exists size_bytes, drop column if exists sha256,
  drop column if exists revision_seq, drop column if exists canonical_storage_status,
  drop column if exists versiya, drop column if exists updated_at,
