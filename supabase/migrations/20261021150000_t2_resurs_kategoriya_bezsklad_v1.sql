...
with n as (
  select btrim(
           regexp_replace(
             regexp_replace(upper(coalesce(p_nom, '')), '[^0-9A-ZА-Я]+', ' ', 'g'),
             '[[:space:]]+', ' ', 'g')) as nom
)
select case
  ...
  when nom ~ '(^| )(TEMIR BETON|TEMIRBETON)( |$)'
    then null::text
  when nom ~ '(^| )(БЕТОН|БЕТОННЫЙ|БЕТОННАЯ|БЕТОННЫЕ|BETON|BETONNIY|BETONNYI|BETONNAYA|BETONLI)( |$)'
       and nom ~ '(^| )(БЛОК|БЛОКИ|БЛОКОВ|ПЛИТА|ПЛИТЫ|ПАНЕЛЬ|ПАНЕЛИ|БОРДЮР|БОРДЮРЫ|КОЛЬЦО|КОЛЬЦА|ТРУБА|ТРУБЫ|ЛОТОК|ЛОТКИ|СТУПЕНЬ|СТУПЕНИ|ФБС|FBS|BLOK|BLOKI|BLOCK|BLOCKS|PLITA|PLITY|PANEL|PANELS|BORDYUR|BORDUR|KOLSO|TRUBA|SMES|СМЕСЬ|ИЗДЕЛИЕ|ИЗДЕЛИЯ|КАМЕНЬ|КАМНИ|КИРПИЧ|КОНСТРУКЦИЯ|ARALASHMA|QORISHMA)( |$)'
    then null::text
