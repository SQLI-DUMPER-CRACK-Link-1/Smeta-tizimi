commit ff5790156da1d0112c5517a359e99faac7f7a891
Author: SQLI DUMPER <sqli-dumper@example.com>
Date:   Tue Aug 4 20:16:49 2026 +0500

    fix: correctly add item to qolBekor when making it a Dop to prevent double counting

diff --git a/frontend/src/admin/sahifalar/F2Import.tsx b/frontend/src/admin/sahifalar/F2Import.tsx
index e252e84..c17eee1 100644
--- a/frontend/src/admin/sahifalar/F2Import.tsx
+++ b/frontend/src/admin/sahifalar/F2Import.tsx
...
function FinancialChart({ objects }: { objects: any[] }) {
  const allSubItems = objects.flatMap(sh => sh.subItems || []).filter(o => !o.nom.startsWith('ЁЯС╖'));
...
function NewsTicker({ objects, jami }: { objects: any[], jami: any }) {
  const msgs = [
    ...
    ...(objects.filter(o => (o.debitor || 0) > 100000000).map(o => `тЪая╕П DIQQAT: ${o.nom} da yirik debitor qarz!`)),
    ...(objects.filter(o => (o.fakt || 0) - (o.f2 || 0) > 50000000).map(o => `тЭДя╕П ${o.nom} da F-2 yopilmagan ishlar mavjud!`))
  ];
  const tickerText = msgs.join("   тАв   ");
        ...
        return (
          <tr key={`rz-${idx}`} className="bg-black/20 border-b border-white/5 hover:bg-black/30 transition-colors">
            <td className="py-3 pl-[80px] pr-4 text-slate-300 font-medium text-sm flex items-center gap-3">
              <span className="text-white/20 font-mono text-lg">тФФ</span>
      ...
      {data.oylar && data.oylar.length > 0 && (
        <tr className="bg-black/30 border-b border-white/10">
          <td colSpan={6} className="py-4 pl-[80px] pr-8">
            <div className="text-xs text-white/40 font-bold mb-3 uppercase tracking-widest">ЁЯУИ F-2 Oylik Dinamikasi</div>
...
function ObyektRow({ obj }: { obj: any }) {
  ...
  const isSub = obj.nom.startsWith('ЁЯС╖');
      ...
      <tr onClick={() => setOpen(!open)} className="cursor-pointer bg-white/[0.01] hover:bg-blue-500/[0.05] transition-all border-b border-white/5">
        <td className="py-4 pl-[48px] pr-4 flex items-center gap-3">
          ...
          <span className={`text-[15px] ${isSub ? 'text-white/50' : 'text-blue-100 font-semibold'}`}>
            ...
            {(obj.fakt - obj.f2) > 0 && <span className="text-[10px] ml-3 text-warn/90 bg-warn/10 px-2 py-0.5 rounded-full border border-warn/20 shadow-[0_0_10px_rgba(251,191,36,0.1)]">тЭДя╕П Muzlagan: <FmtN val={obj.fakt - obj.f2} qisqa /></span>}
          </span>
        </td>
      ...
      <tr onClick={() => setOpen(!open)} className="cursor-pointer bg-blue-500/[0.02] hover:bg-blue-500/[0.08] transition-all border-b border-blue-500/10 shadow-[inset_0_-1px_0_rgba(255,255,255,0.02)]">
        <td className="py-5 px-4 flex items-center gap-3">
          ...
          <div className="flex gap-2 ml-4">
             {shartnoma.tolangan > 0 && <span className="text-[10px] text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-md border border-emerald-400/20 font-mono shadow-[0_0_10px_rgba(52,211,153,0.1)]">ЁЯТ╡ Kesh: <FmtN val={shartnoma.tolangan} qisqa /></span>}
             {shartnoma.debitor > 0 && <span className="text-[10px] text-red-400 bg-red-400/10 px-2 py-1 rounded-md border border-red-400/20 font-mono shadow-[0_0_10px_rgba(248,113,113,0.1)]">тЪа Debitor: <FmtN val={shartnoma.debitor} qisqa /></span>}
          </div>
        </td>
  ...
  return (
    <AuroraBackground>
      ...
      <div className="max-w-[1600px] w-full mx-auto p-6 md:p-8 flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin relative z-10">
        ...
        {/* тЪая╕П 2026-07-31: "Obyektlar Geo-Lokatsiyasi" xaritasi OLIB TASHLANDI тАФ
