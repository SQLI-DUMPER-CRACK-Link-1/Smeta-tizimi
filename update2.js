  ...
  const handleFetchINN = async () => {
    ...
    setTimeout(() => {
      ...
      if (formData.inn === '207111222') {
        ...
        toast('Didox orqali ma\\'lumotlar tortildi!', 'ok');
      } else {
        ...
        toast('Soliq.uz orqali ma\\'lumot topildi', 'ok');
      }
    }, 800);
  };
`;
...
// Add inn to formData state
code = code.replace(/const \[formData, setFormData\] = useState\(\{/, "const [formData, setFormData] = useState({\n    inn: '',");

// Fix reset logic if any
code = code.replace(/setFormData\(\{([\s\S]*?)nom: ''/g, "setFormData({$1inn: '', nom: ''");

// Fix 'success' to 'ok'
code = code.replace(/'success'/g, "'ok'");
code = code.replace(/'info'/g, "'ok'");
